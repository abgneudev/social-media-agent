...
def _sample_high_engagement_posts(net):
...
def _classify_prompt(samples):
    ...
    return (
        ...
        f"post is about. Examples of good topic_angles: 'dark mode contrast', "
        f"'design tokens vs css vars', 'usability test sample sizes'. Examples of "
...
def _classify(ai_generate, samples):
    ...
    if not samples:
        return []
    raw = ai_generate(_classify_prompt(samples))
    if not raw:
        return []
    try:
...
def run(net, ai_generate):
    ...
    samples = _sample_high_engagement_posts(net)
    if not samples:
        logger.info("[ANALYZER] no samples to classify; leaving prior insights.")
        return None
    classifications = _classify(ai_generate, samples)
